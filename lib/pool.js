...
function IsBannedAddress(address) {
    return utils.isBanned(address);
}
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability) {
        ...
        var cn_variant = isVIG && convertedBlob[0] >= 7 ? convertedBlob[0] - 6 : 0;
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage) {
    ...
    if (IsBannedAddress(params.login || miner.login)) {
	console.log('address: ' + (params.login || miner.login) + ' banned!');
        sendReply('your address is banned');
        return;
    }
